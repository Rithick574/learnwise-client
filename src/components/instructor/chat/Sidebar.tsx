...
const Sidebar: FC<SidebarProps> = ({ onUserSelect }) => {
  ...
  console.log("🚀 ~ file: Sidebar.tsx:17 ~ chats:", chats)
    ...
    if (user) {
      try {
        ...
        const fetchedChats = res.data.data.map((chat: any) => {
          const participant = chat.participants.find((p: any) => p._id !== user._id);
          return {
            ...
            profileImageUrl: participant?.profile?.avatar || "../ui/empty-profile.webp",
